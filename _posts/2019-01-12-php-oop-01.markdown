...
# 属性与访问控制
类的变量成员叫做“属性”，或者叫“字段”、“特征”。属性声明是有关键字public、protected或者private开头，然后跟一个普通的变量声明来组成。属性中的变量可以初始化，但是初始化的值必须是常数。
...
# Static (静态)关键字
声明类属性或方法为静态，就可以不实例化类而直接访问。静态属性不能通过一个类已实例化的对象来方法(静态方法可以)。同样静态属性和方法也有访问控制，如果没有指定默认为公有。由于静态方法不需要通过对象即可调用，所以伪变量`$this`在静态方法中不可用。静态属性不可以由对象通过->操作符在访问。
